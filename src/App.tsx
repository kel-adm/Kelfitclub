...
const LoginScreen = () => {
      ...
      if (session?.user && event === 'SIGNED_IN') {
        ...
        try {
          ...
          if (data.token) {
            ...
            setError(data.error || 'Erro ao sincronizar login do Google');
          }
        } catch (err) {
  ...
  const handleGoogleLogin = async () => {
    if (!supabase) {
      setError('Configuração do Google Login ausente (VITE_SUPABASE_URL).');
      return;
    }
    setError('');
    ...
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 text-white">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-white text-black font-bold py-4 rounded-xl active:scale-95 transition-transform disabled:opacity-50">
            {isLoading ? 'Carregando...' : (isRegister ? t.register : t.login)}
          </button>
        </form>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-black px-2 text-white/30 tracking-widest">Ou continue com</span>
          </div>
        </div>
        ...
        <button 
          ...
          disabled={isLoading}
          className="w-full bg-white/5 border border-white/10 text-white font-medium py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-white/10 active:scale-95 transition-all disabled:opacity-50"
        >
